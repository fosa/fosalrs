...
\[ a_n = 0 ~bei ~ungeraden ~ Funktionen ~(sinus)\]
\[ b_n = 0 ~bei ~geraden ~ Funktionen ~(cosinus) \]
...
\[ F(\omega) = |F(\omega)| \cdot e^{j\varphi(\omega)} = A(\omega) \cdot e^{j\varphi(\omega)}   \]
\[ F(\omega): Spektrum ~ von ~ f(t) ~ (Frequenzspektrum, Spektraldichte) \]
\[ A(\omega) = |F(\omega)|: Amplitudenspektrum \]
\[ \varphi (\omega) = arg(F(\omega)): Phasenspektrum \]
...
\subsubsection{Frequenzverschiebung (Modulation)}
